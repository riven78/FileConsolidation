            ...
            if (filelist.size() > 1) {
                Collections.sort(filelist, new Comparator<MyFile>() {
                    ...
                    public int compare(MyFile o1, MyFile o2) {
                        String aa = o1.file.getName().replaceAll("_\\d{13}.",".");
                        String bb = o2.file.getName().replaceAll("_\\d{13}.",".");
                        int ret = aa.length() - bb.length();
                        if (ret == 0) {
                            ret = (int) (o1.file.lastModified() - o2.file.lastModified());
                        }
                        return ret;
                    }
                });
